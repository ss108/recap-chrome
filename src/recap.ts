...
import { getItemsFromStorage, destroyTabStorage } from "./utils";
...
export default function Recap() {
    ...
    return {
        ...
        getAvailabilityForDocket: function (pacerCourt, pacerCaseId, cb) {
            // console.log(pacer)
            if (!pacerCaseId) {
                console.error("RECAP: Cannot get availability of docket without pacerCaseId.");
                return;
            }

            if (!pacerCourt) {
                console.error("RECAP: Cannot get availability of docket without pacerCourt.");
            }

            console.info(`RECAP: Getting availability of docket ${pacerCaseId} at ` +
                `${pacerCourt}`);
            $.ajax({
                ...
                data: {
                    ...
                    court: PACER.convertToCourtListenerCourt(pacerCourt),
        ...
        uploadAttachmentMenu: function (pacer_court, pacer_case_id, html, cb) {
            ...
            // have it.
            pacer_case_id && formData.append('pacer_case_id', pacer_case_id);
            let uploadType = UPLOAD_TYPES['ATTACHMENT_PAGE'] || "";
            formData.append('upload_type', uploadType.toString());
        ...
        uploadDocument: (
            ...
            getItemsFromStorage(cb.tab.id)
                .then(async (tabStorage) => {
                    ...
                    formData.append('upload_type', UPLOAD_TYPES['PDF'].toString());
                    formData.append('debug', DEBUG.toString());
                    return formData;
                })
        ...
        uploadZipFile: (
            ...
            getItemsFromStorage(cb.tab.id)
                .then(async (tabStorage) => {
                    ...
                    formData.append('upload_type', UPLOAD_TYPES['ZIP'].toString());
                    formData.append('debug', DEBUG.toString());
                    formData.append('filepath_local', blob);
                    return formData;
                })
        ...
        uploadClaimsRegister: async function (pacerCourt, pacerCaseId, claimsPageHtml, cb) {
            ...
            formData.append('upload_type', UPLOAD_TYPES['CLAIMS_REGISTER_PAGE'].toString());
