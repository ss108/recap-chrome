...
export function importInstance(constructor: Recap | Notifier) {
  ...
  for (var verb in new constructor()) {
    ;(function (verb) {
      sender[verb] = function () {
        ...
        var unpack = function (results: unknown) {
          cb.apply(null, results)
        }
        chrome.runtime.sendMessage({ name: name, verb: verb, args: args }, unpack)
      }
    })(verb)
  }
  return sender
...
export function httpRequest(
  ...
  callback: (type: string, result: unknown, xhr: InstanceType<typeof XMLHttpRequest>) => void
) {
  let type = null,
    result = null,
    xhr: InstanceType<typeof XMLHttpRequest>
  ...
  try {
    ...
    // @ts-ignore these are special firefox tools without a namespace
...
export async function getItemsFromStorage(key: string | number) {
  let keyAsString = key.toString()
  let res = await chrome.storage.local.get(keyAsString)
  return res[keyAsString]
}

const saveItemToStorage = (dataObj: { [key: string]: any }) =>
  new Promise((resolve, reject) =>
    chrome.storage.local.set(dataObj, () =>
      resolve(console.log(`RECAP: Item saved in storage at tabId: ${Object.keys(dataObj)[0]}`))
    )
  )

const destroyTabStorage = (key: string | number) => {
...
export function getTabIdForContentScript() {
  ...
  return new Promise((resolve) => {
    chrome.runtime.sendMessage(content, (msg) => {
      resolve(msg)
    })
  })
}
...
export async function updateTabStorage(object: Record<string, any>) {
...
$.ajaxSetup({
  ...
  dataType: 'json',
  beforeSend: function (xhr: InstanceType<typeof XMLHttpRequest>, settings: { url: string }) {
...
var DEBUGLEVEL = 1
function debug(level: number, varargs: unknown) {
  if (DEBUGLEVEL >= level) {
...
export function recapAlertButton(court: string, pacerCaseId: string, isActive: boolean) {
  const anchor = document.createElement('a')
  anchor.setAttribute('id', 'recap-alert-button')
  anchor.setAttribute('role', 'button')
  anchor.setAttribute('aria-disabled', `${isActive}`)
  if (!isActive) {
...
export function recapBanner(result: { absolute_url: string; date_modified: string }) {
