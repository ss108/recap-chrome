...
ContentDelegate.prototype.checkRestrictions = function () {
    ...
    if (restrictedDoc) {
        ...
        let target =
            ...
            <Element>document.forms[document.forms.length - 1].lastChild;

        target.insertAdjacentHTML = target.insertAdjacentHTML || function () { };
...
ContentDelegate.prototype.handleSingleDocumentPageCheck = function () {
    ...
    let callback = $.proxy(function (api_results) {
        ...
        let result = api_results.results.filter(function (obj) {
            return obj.pacer_doc_id === this.pacer_doc_id; //pacer_doc_id is otherwise undefined and I hope I am correct in my assessment that it will be a property on the ContentDelegate instance.
        })[0];
        if (!result) {
            return;
        }
...
ContentDelegate.prototype.onDocumentViewSubmit = function (event) {
    ...
    let form = <HTMLFormElement>document.getElementById(event.data.id);
...
ContentDelegate.prototype.showPdfPage = async function (
    ...
    const browserSpecificFetch = window.fetch; //obvs, this variable assignment is now redundant. We can remove later, but just wanted to leave in place to indicate there used to be a ternary operator here. This was eliminated by whatwg-fetch polyfill imported at the top.
